        ...
        $("#addrow").on("click", function() {
            ...
            <select class="form-control form-select" name="row[` + counter + `][product]" id="situation_familialev" required>
                                            @isset($data['products'])
                                                    ...
                                                    <option style="background:#f9cfcb; color:black" value="{{$item->id}}">{{$item->name}}  ({{$item->physical_quantity}}) </option>
                                                    @elseif($item->physical_quantity<=$item->qte_min)
                                                    <option style="background:#fcedbc; color:black" value="{{$item->id}}">{{$item->name}}  ({{$item->physical_quantity}}) </option>
                                                    @else
                                                    <option style="background:#c0f6e8; color:black" value="{{$item->id}}">{{$item->name}}  ({{$item->physical_quantity}}) </option>
                                                    @endif
...
<div class="nk-content-body">
    <form id="" method="post" action="{{route('bcmd.store')}}" autocomplete="off" enctype="multipart/form-data">
        @csrf()
        <div class="nk-block-head nk-block-head-sm">
            <div class="nk-block-between">
                <div class="nk-block-head-content">
                    <h3 class="nk-block-title page-title">Ajouter un bon de commande</h3>
